...
int main(int argc, char **argv) {
    ...
    QTemporaryDir temp; REQUIRE(temp.isValid()); const QString path = temp.filePath("model.pcad"); auto saveOne = DocumentStorage::save(path, d.record(), std::nullopt); REQUIRE(saveOne.ok); const auto stable = d.record(); auto changed = stable; changed.revision++; auto saveTwo = DocumentStorage::save(path, changed, stable.revision); if (!saveTwo.ok) std::fprintf(stderr, "save error: %s\n", qPrintable(saveTwo.error)); REQUIRE(saveTwo.ok); REQUIRE(!DocumentStorage::save(path, stable, stable.revision).ok);
