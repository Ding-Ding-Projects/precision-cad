...
Containment uses exact line segment tests and analytic circle tests. The bounded tolerance is used only to conservatively mark boundary contact as ambiguous or touching. Output remains analytic; it never uses a chord approximation for returned geometry.
