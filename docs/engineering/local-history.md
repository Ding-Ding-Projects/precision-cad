...
Restore begins with a validated preview of a revision and path. The preview binds SHA-256 digests of both the active and historical bytes. Apply takes a local file lock, refuses if either bound input changed, copies the current working document to an exclusive timestamped sibling first, validates matching document identity, advances the active revision, then atomically writes the replacement. It never resets, checks out, deletes, switches refs, overwrites the only current document without preservation, or modifies Git history.
